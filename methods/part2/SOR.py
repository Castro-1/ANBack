...
def sor_method(A, b, x0, omega, tol, max_iter, error):
    L = np.tril(A)
    U = A - L
    x = x0
    convergence = 0
...
def solve_sor(A, b, x0, omega, tol, max_iter, error):
    spectral_r = spectral_radius(A)
    sol, convergence, n_iter, x_values = sor_method(A, b, x0, omega, tol, max_iter, error)
    sol = sol.tolist()
